	...
	<div id="main">
		<div>
			States :
			 ...
			 City:
			 <select id="city" onChange={(e)=>setCity(e.target.value)}>{states[state].city.map((item,index)=>
			 <option key={`city${index+1}`} value={index}>{item.name}</option>
			 )}</select>
			 Towns:
			 <select id="lendmark" onChange={(e)=>setTown(e.target.value)}>{states[state].city[city].landmarks.map((item,index)=>
